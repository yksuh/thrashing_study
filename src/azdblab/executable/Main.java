...
//		_logger.outputLog("effective db size = " + String.format("%.2f", effDBSz));
		...
		String str = String.format("DBMSBufferCacheSize: %.2f%%, %d%%, %.2f%%", dbmsBuffCacheSizeMin*100, dbmsBuffCacheSizeIncr*100, dbmsBuffCacheSizeMax*100);
		...
		_logger.outputLog(str);
		str = String.format("Exclusive Lock Pct: %d%%, %d%%, %d%%", eXclusiveLockPctMin*100, eXclusiveLockPctIncr*100, eXclusiveLockPctMax*100);
		_logger.outputLog(str);
		str = String.format("Multiprogramming Level: %d, %d, %d", MPLMin, MPLIncr, MPLMax);
		_logger.outputLog(str);
		str = String.format("Effective DB: %d%%, %d%%, %d%%", EffDBMin*100, EffDBIncr*100, EffDBMax*100);
		_logger.outputLog(str);
		MPLMin = experiment.getMPLMin();
		MPLMax = experiment.getMPLMax();
		MPLIncr  = experiment.getMPLIncr();
