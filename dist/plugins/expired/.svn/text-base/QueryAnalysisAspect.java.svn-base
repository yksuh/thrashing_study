...
//import azdblab.AZDBLAB;
//import azdblab.analysis.dataDefinition.ForeignKey;
//import azdblab.dbms.api.GeneralDBMS;
...
public class QueryAnalysisAspect extends Aspect {
	...
	private	static final String			strStyleName		= "QueryAnalysisStyle";
	
	private JPanel 						panelStyleSelect;
	
	
	public QueryAnalysisAspect( 
							String user_name, 
	...
	protected void createStyleTable() {
		...
		TABLE_DEFINEDASPECTOFSTYLE =
			myDBController.new InternalTable(
					...
					new String[] { "UserName", "NotebookName", "AspectName", "Description" },
	...
	protected void getAspectDetails() {
		...
		/*
		String 					selectColumnNames	= TABLE_DEFINEDASPECTOFSTYLE.columns[4];	
		...
		try{
			...
			while (rs.next()) {
				...
				strAspeDescription 	= rs.getString(1);
	...
	protected boolean storeDefinedAspect() {
		insertAspect(strUserName, strNotebookName,
				strAspectName, getAspectStyleName(), strAspectDescription,
				strAspectSQL);
		return true;
	}
	...
	public boolean storeSpecification() {
		...
		return false;
		/*
		String[]	columnValues	= new String[TABLE_DEFINEDASPECTOFSTYLE.columns.length - 1];
		
		columnValues[0]				= strUserName;
		columnValues[1]				= strNotebookName;
		columnValues[2]				= strAspectName;
		//columnValues[3]				= strAspectValue;
		//columnValues[3]				= strDescription;
		//columnValues[4]				= strSQL;
		
		String[]	columnNames		= new String[TABLE_DEFINEDASPECTOFSTYLE.columns.length - 1];
		int[]		columnDataTypes	= new int[TABLE_DEFINEDASPECTOFSTYLE.columns.length - 1];
		
		for (int i = 0; i < TABLE_DEFINEDASPECTOFSTYLE.columns.length - 1; i++) {
			columnNames[i]		= TABLE_DEFINEDASPECTOFSTYLE.columns[i];
			columnDataTypes[i]	= TABLE_DEFINEDASPECTOFSTYLE.columnDataTypes[i];
		}
		
		return myDBController.insertTuple(	TABLE_DEFINEDASPECTOFSTYLE.TableName,
									TABLE_DEFINEDASPECTOFSTYLE.columns[TABLE_DEFINEDASPECTOFSTYLE.columns.length - 1],
									columnNames,
									columnValues,
									columnDataTypes );
		*/
