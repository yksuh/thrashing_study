 ...
 * 
 */
public class OnePassWithoutTimingScenario extends ScenarioBasedOnQuery {
	/**
	 * @param expRun
	 *            experiment run instance
	 */
	public OnePassWithoutTimingScenario(ExperimentRun expRun) {
		super(expRun);
		NUMQUERYEXECS = 1;
	}
...
//					state.query, state.pstmt, state.plan, state.card);
			...
			curr_query_execution.exec_time = 0;
			curr_query_execution.proc_diff_ = "N/A";
...
//			}
			 ...
			 */
			if(curr_query_execution.exec_time == Constants.MAX_EXECUTIONTIME){
				break;
			}
		}
