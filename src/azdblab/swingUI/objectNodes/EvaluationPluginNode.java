...
public class EvaluationPluginNode extends ObjectNode {

	private String strDateCreate;

	public EvaluationPluginNode() {
		strNodeName = "Evaluation Plugins";
	}
	...
	private JPanel createScenarioPanel() {
		...
		info += "All the Evaluation Plugins";
		info += "</font></BODY></HTML>";

		NodePanel npl_toRet = new NodePanel();
		npl_toRet.addComponentToTab("Evaluation Plugin Info",  createTextPaneFromString(info));
	...
	protected void loadChildNodes() {
		loadChilds(Main.masterManager
				.getPluginsWithSuperclass(azdblab.plugins.evaluation.Evaluation.class));
	...
	private void loadChilds(Vector<PluginData> myPlugins) {
		for (int i = 0; i < myPlugins.size(); i++) {
			try {
				if (Modifier.isAbstract(myPlugins.get(i).pluginClass
						.getModifiers())) {
					return;
				}
				...
				String styleName = myPlugins.get(i).pluginClass.getSimpleName()
						.replaceAll("Scenario", "")
						+ "_" + version_tag;
