...
public abstract class ScenarioBasedOnCardinality extends Scenario {
	/**
	 * @param expRun
	 *            experiment run instance
	 */
	public ScenarioBasedOnCardinality(ExperimentRun expRun) {
		super(expRun);
	...
	protected QueryExecutionStat timeQueryExecution(String sql, PlanNode plan, long cardinality) throws Exception {
		...
		return result_queryrxecution_stat;
	}
	/**
...
if (Main.verbose) {
	Main._logger.outputLog("Running Query # " + (j+1) + "-" + t + " iteration(s)");
					 ...
					 */
					if(qrstat.getQueryTime() == Constants.MAX_EXECUTIONTIME){
						break;
					}
				}
			}
