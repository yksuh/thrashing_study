...
INSERT INTO __Analysis_RowCount_Ver1__ (dbmsName, exprName, stepName, stepResultSize)
	SELECT dbms as dbmsName, 
	       ...
	       '__Analysis_S2_I_Ver1__' as stepName,
	       COUNT(*) as stepResultSize
	FROM __Analysis_S2_I_Ver1__
	GROUP BY dbms, experimentname;
DROP TABLE __Analysis_S2_II_Ver1__ CASCADE CONSTRAINTS;
CREATE TABLE __Analysis_S2_II_Ver1__ AS
	SELECT  *
	FROM __Analysis_S2_I_Ver1__
	WHERE
	     (numphantomspresent = 0 AND stoppedprocesses=0);	
ALTER TABLE __Analysis_S2_II_Ver1__ ADD PRIMARY KEY (qeid); 
INSERT INTO __Analysis_RowCount_Ver1__ (dbmsName, exprName, stepName, stepResultSize)
	SELECT dbms as dbmsName, 
	       experimentname as exprName,
	       '__Analysis_S2_II_Ver1__' as stepName,
	       COUNT(*) as stepResultSize
	FROM __Analysis_S2_II_Ver1__
	GROUP BY dbms, experimentname;
DROP VIEW __Analysis_S2_III_Ver1__ CASCADE CONSTRAINTS;
CREATE VIEW __Analysis_S2_III_Ver1__ AS
	SELECT t1.*
        FROM __Analysis_S2_II_Ver1__ t1,
             (SELECT DISTINCT
			dbms,
			runid, 
  		        querynum, 
                        card, 
			MEDIAN(iowait) as median_iowt
              FROM __Analysis_S2_II_Ver1__ 
	      WHERE dbms IN ('oracle', 'pgsql')
              GROUP BY dbms, runid, querynum, card) t2
        WHERE t1.dbms = t2.dbms AND
	      ((median_iowt>0 AND iowait <= 2*median_iowt) OR (median_iowt=0 AND iowait <= 2)) AND
	...
	SELECT *
        FROM __Analysis_S2_II_Ver1__ t1
        WHERE dbms IN ('db2', 'mysql');
ALTER VIEW __Analysis_S2_III_Ver1__ ADD PRIMARY KEY (qeid) DISABLE; 
INSERT INTO __Analysis_RowCount_Ver1__ (dbmsName, exprName, stepName, stepResultSize)
	SELECT dbms as dbmsName, 
	       experimentname as exprName,
	       '__Analysis_S2_III_Ver1__' as stepName,
	       COUNT(*) as stepResultSize
	FROM __Analysis_S2_III_Ver1__
	GROUP BY dbms, experimentname;
DROP VIEW __Analysis_S2_IV_Ver1__ CASCADE CONSTRAINTS;
CREATE VIEW __Analysis_S2_IV_Ver1__ AS
	SELECT *
	FROM __Analysis_S2_III_Ver1__	
	MINUS
	SELECT *
        FROM __Analysis_S2_III_Ver1__ t1
        WHERE
	      ((dbms = 'db2' AND iowait > 0) OR (dbms = 'mysql' AND iowait*10 > CEIL(measured_time*0.01)));
ALTER VIEW __Analysis_S2_IV_Ver1__ ADD PRIMARY KEY (qeid) DISABLE; 
INSERT INTO __Analysis_RowCount_Ver1__ (dbmsName, exprName, stepName, stepResultSize)
	SELECT dbms as dbmsName, 
	       experimentname as exprName,
	       '__Analysis_S2_IV_Ver1__' as stepName,
	       COUNT(*) as stepResultSize
	FROM __Analysis_S2_IV_Ver1__
	GROUP BY dbms, experimentname;
...
CREATE TABLE __Analysis_CDP_Ver1__ AS
	 ...
	 FROM __Analysis_S2_IV_Ver1__ s,
...
INSERT INTO __Analysis_RowCount_Ver1__ (dbmsName, exprName, stepName, stepResultSize)
	SELECT dbms as dbmsName, 
	       experimentname as exprName,
	       '__Analysis_S3_I_Ver1__' as stepName,
	       COUNT(*) as stepResultSize
...
INSERT INTO __Analysis_RowCount_Ver1__ (dbmsName, exprName, stepName, stepResultSize)
	SELECT dbms as dbmsName, 
	       experimentname as exprName,
	       '__Analysis_S3_II_Ver1__' as stepName,
	       COUNT(*) as stepResultSize
...
INSERT INTO __Analysis_RowCount_Ver1__ (dbmsName, exprName, stepName, stepResultSize)
	SELECT dbms as dbmsName, 
	       experimentname as exprName,
	       '__Analysis_S3_III_Ver1__' as stepName,
	       COUNT(*) as stepResultSize
	FROM __Analysis_S3_III_Ver1__
	GROUP BY dbms, experimentname;
DROP VIEW __Analysis_PI_Ver1__ CASCADE CONSTRAINTS;
CREATE VIEW __Analysis_PI_Ver1__ AS
	SELECT DISTINCT 
	       qe.version,
	       qe.experimentid,
	       qe.experimentname,
	...
	FROM __Analysis_S3_III_Ver1__ qatc, 
	     __Analysis_S2_IV_Ver1__ qe,
	     AZDBLab_QueryExecutionProcs qp
	WHERE qatc.runid = qe.runid AND 
	      qatc.querynum = qe.querynum AND
	      qatc.card = qe.card AND
	      qe.qeid = qp.queryexecutionid;
ALTER VIEW __Analysis_PI_Ver1__ ADD PRIMARY KEY (qeid, procid) DISABLE; 
DROP VIEW __Analysis_QPI_Ver1__ CASCADE CONSTRAINTS;
CREATE VIEW __Analysis_QPI_Ver1__ AS
	SELECT qeid,
	       uticks,
	       sticks,
	       maj_flt
...
ALTER VIEW __Analysis_QPI_Ver1__ ADD PRIMARY KEY (qeid) DISABLE;
...
DROP VIEW __Analysis_UPI_Ver1__ CASCADE CONSTRAINTS;
CREATE VIEW __Analysis_UPI_Ver1__ AS
	SELECT * FROM __Analysis_EUPI_Ver1__
	UNION
	SELECT * FROM __Analysis_NUPI_Ver1__;
ALTER VIEW __Analysis_UPI_Ver1__ ADD PRIMARY KEY (qeid) DISABLE;
...
DROP VIEW __Analysis_DPI_Ver1__ CASCADE CONSTRAINTS;
CREATE VIEW __Analysis_DPI_Ver1__ AS
	SELECT * FROM __Analysis_EDPI_Ver1__
	UNION
	SELECT * FROM __Analysis_NDPI_Ver1__;
ALTER VIEW __Analysis_DPI_Ver1__ ADD PRIMARY KEY (qeid) DISABLE; 
...
INSERT INTO __Analysis_RowCount_Ver1__ (dbmsName, exprName, stepName, stepResultSize)
	SELECT dbms as dbmsName, 
	       experimentname as exprName,
	       '__Analysis_QED_Ver1__' as stepName,
	       COUNT(*) as stepResultSize
...
INSERT INTO __Analysis_RowCount_Ver1__ (dbmsName, exprName, stepName, stepResultSize)
	SELECT dbms as dbmsName, 
	       experimentname as exprName,
	       '__Analysis_S4_CTQatC_Ver1__' as stepName,
	       COUNT(*) as stepResultSize
