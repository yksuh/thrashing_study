...
public class BouncingScenario extends VaryCardinalityScenario {
	public BouncingScenario(ExperimentRun expRun) {
		super(expRun);
	}
	...
	protected QueryExecutionStat stepB(int phaseNumber, String sql,
		...
		try {
			Process p = Runtime.getRuntime().exec("/opt/oracle/orabounce");
			InputStreamReader isr = new InputStreamReader(p.getInputStream());
			BufferedReader br = new BufferedReader(isr);
			String ss = null;
			while ((ss = br.readLine()) != null) {
				// Main.defaultLogger.logging_info(ss);
				ss = ss + ""; // Note: Just to remove the warning that ss is
				// never used.
			}
	 ...
	 *         time.
	 */
	protected QueryExecutionStat stepD(int phaseNumber, String sql,
			PlanNode plan, long cardinality) throws Exception {
		experimentSubject.close();
		try {
			Process p = Runtime.getRuntime().exec("/opt/oracle/orabounce");
		...
		// if (planToExecutionTimeMap.containsKey(plan.myHashCode())) {
		// Main.defaultLogger.logging_info("Plan executed before, using old timing result");
		// return new
		// QueryExecutionStat(planToExecutionTimeMap.get(plan.myHashCode()));
		// } else {
		QueryExecutionStat result_queryexecutionstat = timeQueryExecution(sql,
				plan, cardinality);
		planToExecutionTimeMap.put(plan.myHashCode(), result_queryexecutionstat
	...
	protected void setName() {
		scenarioName = Constants.NAME_BOUNCING_SCENARIO;
	}

	@Override
	protected void setVersion() {
		versionName = Constants.VERSION_BOUNCING_SCENARIO;
	}
